...
\begin{frame}
  ...
  \begin{figure}[ht]
    ...
    \begin{tikzpicture}[scale=1, every node/.style={font=\small}]
      % Linux box
      \draw[thick, fill=green!10] (-2,0.4) rectangle (5.9,2.25);
      \node at (3,2) {Linux (Docker Engine)};
      ...
      \node[draw, fill=orange!20, rounded corners] (server) at (3,1) {VS Code (Zerbitzaria)};
      % VS Code (Client) outside, arrow to DevContainer
      \node[draw, fill=orange!20, rounded corners, align=center] (client) at (-0.8,1) {VS Code\\(Bezeroa)};
      \draw[-] (client) -- (server);
    \end{tikzpicture}
    \caption{Arkitektura Linux-en (Docker Engine)}
    \label{fig:devcontainer_linux}
  \end{figure}
\end{frame}
...
\begin{frame}
  ...
  \begin{figure}[ht]
    ...
    \begin{tikzpicture}[scale=1, every node/.style={font=\small}]
      ...
      \node at (3,3) {Windows 11 (Hyper-V)};
      % WSL2 (VM) box inside Windows
      \draw[thick, fill=cyan!10] (0.4,0.2) rectangle (5.6,2.75);
      \node at (3,2.5) {WSL2 (VM)};
      % WSL2 distro box inside WSL2 (VM)
      \draw[thick, fill=green!10] (0.7,0.4) rectangle (5.3,2.25);
      \node at (3,2) {WSL2 distro (docker-desktop)};
      % DevContainer box inside WSL2 distro
      \draw[thick, fill=yellow!20] (1,0.6) rectangle (5,1.75);
      \node at (3,1.5) {DevContainer};
      % VS Code Server box inside WSL2 distro
      ...
      \draw[-] (client) -- (server);
    \end{tikzpicture}
    \caption{Arkitektura Windows-en (WSL2)}
    \label{fig:devcontainer_windows}
  \end{figure}
\end{frame}
